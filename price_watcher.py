...
def fetch_data(symbol):
    try:
        ...
        df = df.iloc[::-1]  # ترتيب تصاعدي (الأقدم إلى الأحدث)
...
# بدء التتبع
send_to_telegram("✅ بدأ التتبع لأسعار ETH و GOLD")
...
while True:
    ...
    if price_eth:
        if last_price_eth is None:
            ...
            change = abs(price_eth - last_price_eth) / last_price_eth * 100
            if change >= 1:
                emoji = "📈" if price_eth > last_price_eth else "📉"
                send_to_telegram(f"{emoji} ETH السعر: ${price_eth:.2f} | تغير: %{change:.2f}\nRSI: {rsi_eth:.2f} | MACD: {macd_eth:.2f}")
                last_price_eth = price_eth
    ...
    if price_gold:
        if last_price_gold is None:
            ...
            change = abs(price_gold - last_price_gold)
            if change >= 5:
                emoji = "📈" if price_gold > last_price_gold else "📉"
                send_to_telegram(f"{emoji} GOLD السعر: ${price_gold:.2f} | فرق: ${change:.2f}\nRSI: {rsi_gold:.2f} | MACD: {macd_gold:.2f}")
                last_price_gold = price_gold
